...
- Если характеристики нет среди specs/фактов источника — не утверждай ни наличие,
  ни отсутствие: формулировку про неё в description/bullets не пиши вообще.
  Эталон нарушения: «защита от протечек» / «частичная защита от протечек (корпус)»
  при отсутствии этой строки в facts/annotation. То же для «функция пара»,
  «беспроводное подключение» и любой «защита от X» / «функция X», которой нет
  в данных карточки.
...
- Не называй фирменные функции (AquaProtect, AquaStop, AQUAPROTECT), если их нет
  ...
  иначе не упоминай тему вовсе (см. правило выше).
- Материал бака и материал барабана — разные факты. Пиши каждый только из своего
  поля specs (материал_бака / материал_барабана). Не копируй значение одного
  на другое по аналогии: «бак из нержавеющей стали» при материал_бака=пластик —
  ошибка, даже если барабан действительно из стали.

  ...
  const parts = issues.map(({ field, reason }) => {
    ...
    if (field === 'description') {
      ...
      if (/пунктуац/i.test(reason)) {
        return 'description: между фактами нужна точка («16 программ. Скорость отжима…», «16 программ стирки. Максимальная…»); не ставь точку после связки «Предусмотрены»/«Есть»; элементы списка через запятую';
      }

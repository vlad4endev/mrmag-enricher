...
t('плюсы товара идут отдельным списком после характеристик', () => {
  const [p] = buildV2([v2row('1', { объем_л: 25 }, { bullets: ['Гриль 1200 Вт', '  ', 'Блокировка от детей'] })]).products;
  assert.strictEqual(p.description_html,
    '<p>Коротко</p><ul><li>Объем: 25 л</li></ul>'
    + '<ul><li>Гриль 1200 Вт</li><li>Блокировка от детей</li></ul><p>Подробно</p>',
